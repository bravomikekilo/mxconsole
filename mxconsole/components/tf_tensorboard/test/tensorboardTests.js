...
==============================================================================*/
describe('tf-tensorboard tests', function () {
    window.HTMLImports.whenReady(function () {
        ...
        it('specified tabs are correct', function (done) {
            setTimeout(function () {
                ...
                var tabMode = Array.prototype.map.call(tabs, function (x) { return x.dataMode; });
                chai.assert.deepEqual(tabMode, TF.Globals.TABS, 'mode is correct');
                var tabText = Array.prototype.map.call(tabs, function (x) { return x.innerText.toLowerCase(); });
        ...
        it('renders injected content', function () {
            var injected = tensorboard.querySelector('#inject-me');
            chai.assert.isNotNull(injected);
        });
        describe('reloading the selected dashboard', function () {
            TF.Globals.TABS.forEach(function (name, tabIndex) {
                ...
                it(name + ": calling reload reloads dashboard", function (done) {
                    ...
                    setTimeout(function () {
                        ...
                        tensorboard.reload();
                        chai.assert.isFalse(tensorboard.$$('#reload-button').disabled, 'reload button not disabled');
        ...
        it('reload is disabled for graph dashboard', function (done) {
            var idx = TF.Globals.TABS.indexOf('graphs');
            ...
            setTimeout(function () {
                ...
                tensorboard.reload();
                chai.assert.isTrue(tensorboard.$$('#reload-button').disabled, 'reload button disabled');
